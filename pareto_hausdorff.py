...
def main():
    ...
    # Calculate gd, igd, hausdorff distances
    hausdorff_distances = [average_hausdorff_distance(list(points[0]), list(pareto_final)) for points in pareto_list_of_sets] # Calculate the average hausdorff distance between the pareto_final and every point in the point group
    gds = [distance(list(points[0]), list(pareto_final)) for points in pareto_list_of_sets] # Calculate the average hausdorff distance between the pareto_final and every point in the point group
    igds = [distance(list(pareto_final), list(points[0])) for points in pareto_list_of_sets] # Calculate the average hausdorff distance between the pareto_final and every point in the point group
    print("Hausdorff distances")
    for file, gd, igd, hd in zip(files, gds, igds, hausdorff_distances):
        print(f"{file}: GD: {gd:.2f}, IGD: {igd:.2f}, Hausdorff: {hd:.2f}")
